...
class _NewsPageState extends State<NewsPage> {
  ...
  Future<List<BeanArticleDiscounted>> discountedProduit() async {
    final url = Uri.parse('${dotenv.env['URL']}backendcommerce/getarticlesdiscounted');
    mreponse.Response response = await client.get(url);
    if(response.statusCode == 200){
      _isLoading = ++callNumber == 2 ? false : true;
      List<dynamic> body = jsonDecode(const Utf8Decoder().convert(response.bodyBytes));
  ...
  Future<List<BeanArticleBooked>> getmobilearticlebookedbycompany(int idCompany) async {
    final url = Uri.parse('${dotenv.env['URL']}backendcommerce/getmobilearticlebookedbycompany');
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
        future: Future.wait([_checkData()]),
        builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
          if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
            User? mUser = snapshot.data[0];
            if((mUser != null) && (mUser.idcli > 0) && (mUser.nom.isEmpty)){
              adminCompId = mUser.idcli;
              return Scaffold(
                  ...
                  body: FutureBuilder(
                      future: Future.wait([getmobilearticlebookedbycompany(adminCompId)]),
                      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                        if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
                                          ...
                                          Navigator.push(
                                              ...
                                              MaterialPageRoute(builder: (context) {
                                                  return DetailCommandeEcran.setAllNew(
                                                      liste[index].dates, liste[index].heure, client,
                                                      liste[index].iduser
                                                  );
                                                }
                                              )
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(
                                            children: [
                                              Container(
                                                alignment: Alignment.topLeft,
                                                child: Text("${liste[index].nom} ${liste[index].prenom}",
                                              ...
                                              Container(
                                                alignment: Alignment.topLeft,
                                                child: Text("${liste[index].dates}  -  ${liste[index].heure}",
                                                  style: const TextStyle(
                                                      fontSize: 15
                                                  ),
                                                ),
                                              ...
                                              Container(
                                                alignment: Alignment.topLeft,
                                                child: Text("Total article : ${liste[index].totaux}",
                                                  style: const TextStyle(
                                                      fontSize: 15
                                                  ),
                                                )
              ...
              return Scaffold(
                  ...
                  body: <Widget>[
                    SingleChildScrollView(
                      child: FutureBuilder(
                        future: Future.wait([produitLoading(), recentProduitLoading(), discountedProduit()]),
                        builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
                          if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
                            // Get LIST of OBJECTs
                                ...
                                Column(
                                  children: [
                                    ...
                                    SizedBox(
                                      width: MediaQuery.of(context).size.width,
                                      height: 180,
                                      child: DiscountedProduitInterface(liste: disA, client: client),
                                    )
                                  ],
                                )
                                ,
